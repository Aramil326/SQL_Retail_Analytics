...
SELECT * FROM groups_sorted_by_affinity_index;

DROP FUNCTION IF EXISTS get_frequency_of_visits CASCADE ;
CREATE OR REPLACE FUNCTION get_frequency_of_visits (first_date TIMESTAMP, last_date TIMESTAMP, added_transactions INT,
                                maximum_churn_index NUMERIC, maximum_share_of_transactions NUMERIC, allowable_margin_share NUMERIC)
RETURNS TABLE (
    Customer_ID INT,
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        round((abs(EXTRACT(EPOCH FROM first_date - last_date) / 86400)) /
              (SELECT customer_frequency FROM customers WHERE customers.customer_id = groups."Customer_ID")) + added_transactions,
        ...
        JOIN groups_sku ON groups_sku.group_id = groups."Group_ID"
            ...
            AND ceil(groups."Group_Minimum_Discount" * 20) * 5
                 < ((SELECT avg_margin FROM average_margin WHERE average_margin.group_id = groups_sku.group_id) * allowable_margin_share);
END;
$$ LANGUAGE plpgsql;

SET datestyle = 'european';

SELECT * FROM get_frequency_of_visits('2022-08-18 00:00:00', '2022-08-18 00:00:00', 1,3, 70, 30);
