CREATE OR REPLACE FUNCTION sku_share_in_group(customer_id_ INT, group_id_ INT, sku_id_ INT)
    RETURNS NUMERIC AS
$skgr$
BEGIN
    RETURN
            100 * CAST((SELECT count(DISTINCT t.transaction_id)
                  ...
                  WHERE customer_id = customer_id_
                    ...
                    AND ch.sku_id = sku_id_) AS NUMERIC) /
            (SELECT count(DISTINCT t.transaction_id)
             ...
             WHERE customer_id = customer_id_
               AND sku.group_id = group_id_);
END;
$skgr$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION margin_growth_offers(groups_number INT,
